...
public class Semantic {
	...
	public Semantic(AST.program program) {
		...
		List<Error> errors = classTable.getErrors(); // ClassTable cannot access reportError method (its not static).
														// Thus, errors are returned in a list (from classTable) and
														// printed.
		for (Error e : errors) {
		...
		for (AST.class_ e : program.classes) {
			...
			scopeTable.insert("self", new AST.attr("self", e.name, new AST.no_expr(e.lineNo), e.lineNo)); // self is
																											// available
																											// as
																											// attribute
																											// within
																											// the class
			scopeTable.insertAll(classTable.getAttrs(e.name)); // insert all inherited and other declared attributes
																// within the class into the scope
			processNode(e);
	...
	private void processGraph(List<AST.class_> classes) {
		...
		/*
		 * Checking for : - bad redefinitions - bad inheritance Also : assigning an
		 * integer corresponding to each class.
		 */
		...
		/*
		 * We are creating an undirected graph in this method. Also: Checking for -
		 * undefined parent class error
		 */
		for (AST.class_ e : classes) {
		...
		while (!q.isEmpty()) {
			...
			if (visited[u] == false)
				...
				reportError(idxCont.get(idxName.get(u)).filename, 1, "Class " + idxName.get(u) + ", or an ancestor of "
						+ idxName.get(u) + ", is involved in an inheritance cycle.");
		...
		if (cycles)
			System.exit(1); // exit if cycles found
		...
		while (!q.isEmpty()) {
			int u = q.poll();
			if (u != 1 && u != 0) {
				classTable.insert(idxCont.get(idxName.get(u))); // insert classes in BFS-order so that methods and
	...
	private void processNode(AST.class_ class_) {
		/*
		 * The method checks if the features are attr or method. Corresponding
	...
	private void processNode(AST.method method) {
		...
		for (AST.formal e : method.formals) {
			...
			/*
			 * Error: repeat formal parameters : Example func(x : Int, y : Bool, x : String)
			 * : Object { ... }// x is multiply defined
			 */
		...
		if (classTable.conforms(method.body.type, method.typeid) == false) {
			reportError(filename, method.body.lineNo, "Inferred return type " + method.body.type + " of method "
	...
	private void processNode(AST.attr attr) {
		...
		if (attr.value.getClass() != AST.no_expr.class) {
			...
			if (classTable.conforms(attr.value.type, attr.typeid) == false) {
				reportError(filename, attr.value.lineNo,
						"Inferred type " + attr.value.type + " of initialization of attribute " + attr.name
	...
	private void processNode(AST.assign assign) {
		...
		/*
		 * Errors: - Variable has not been declared - Type of expression being assigned
		 * to the variable does not conform to the type of the variable
		 */
		if (a == null)
			...
			reportError(filename, assign.lineNo,
					"Type " + assign.e1.type + " of assigned expression does not conform to declared type " + a.typeid
							+ " of identifier " + a.name);
	...
	private void processNode(AST.static_dispatch sd) {
		...
		if (c == null)
			...
			reportError(filename, sd.lineNo, "Expression type " + sd.caller.type
					+ " does not conform to declared static dispatch type " + c.name);
		else {
				...
				if (sd.actuals.size() != m.formals.size())
					...
					for (int i = 0; i < sd.actuals.size(); ++i) {
						...
						if (classTable.conforms(actual_type, formal_type) == false)
							reportError(filename, sd.lineNo, "In call of method " + m.name + ", type " + actual_type
	...
	private void processNode(AST.dispatch dispatch) {
		...
		if (c == null) {
			...
			if (c.mlist.containsKey(dispatch.name)) {
				// System.out.println("Found the method " + dispatch.name + " in class " +
				// c.name);
				found = true;
				...
				if (dispatch.actuals.size() != m.formals.size())
					reportError(filename, dispatch.lineNo,
							"Method " + m.name + " invoked with wrong number of arguments.");
				else {
					for (int i = 0; i < dispatch.actuals.size(); ++i) {
						String actual_type = dispatch.actuals.get(i).type;
						String formal_type = m.formals.get(i).typeid;
						if (classTable.conforms(actual_type, formal_type) == false)
							reportError(filename, dispatch.lineNo, "In call of method " + m.name + ", type "
									+ actual_type + " does not conform to declared type " + formal_type);
					}
				}
			} else {
	...
	private void processNode(AST.let let) {
		if (let.value.getClass() != AST.no_expr.class) {
			...
			if (classTable.conforms(let.value.type, let.typeid) == false)
				reportError(filename, let.lineNo, "Inferred type of " + let.value.type + " of initialization" + "of "
						+ let.name + " does not conform to idenitifier's declared type " + let.typeid);
		}
		scopeTable.enterScope();
		scopeTable.insert(let.name, new AST.attr(let.name, let.typeid, let.value, let.lineNo));
		processNode(let.body);
		/*
		 * testing.cl:1: Inferred type String of initialization of b does not conform to
		 * identifier's declared type Int.
		 */
	...
	private void processNode(AST.typcase typcase) {
		...
		for (AST.branch e : typcase.branches) {
			...
			if (c == null) {
				...
				scopeTable.insert(e.name, new AST.attr(e.name, "Object", e.value, e.lineNo)); // In the case of
																								// erroneous branch
																								// type, branch variable
																								// has type "Object" for
																								// the scope.
			} else
	...
	private void processNode(AST.string_const string_const) {
		if (!GlobalVariables.GlobalStringToIRMap.containsKey(string_const.value)) {
			GlobalVariables.GlobalStringToIRMap.put(string_const.value, "@.str." + GlobalVariables.GlobalStringCounter);
